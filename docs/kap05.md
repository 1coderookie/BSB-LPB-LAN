...
|  `/DG`                  | `Grafische Anzeige der Logdatei` <br /> `Wer Parameter auf SD-Karte loggt, hat neben der reinen Textform auch die Möglichkeit, einen Graphen angezeigt zu bekommen.` <br /> `Hinweis: Für /DG muss bei Javascript-Blockern die Domain d3js.org freigegeben werden, da der Arduino weiterhin nur die CSV-Datei in den Browser lädt und diese dann mit dem D3-Framework grafisch aufbereitet wird.` <br /> `Wird die Log-Datei via Webinterface mittels Klick auf „Anzeige Logdatei" aufgerufen, erfolgt standardmäßig zuerst die grafische Darstellung. Mouseover-, Klick- und Mausradaktionen innerhalb der grafischen Darstellung bieten diverse Steuerungsmöglichkeiten, wie z.B. interaktives Hervorheben oder Ausschalten von Plotlinien, vertikale Skalierung sowie Zoom- und Pan-Funktionen. `   
